<template>
  <article class="bg-white rounded-xl border border-leaf-100 p-4 sm:p-5 hover:border-leaf-300 transition-colors">
    <div class="flex items-center gap-2 text-xs text-stone-400 flex-wrap">
      ...
      <NuxtLink v-if="item.score != null" :to="`/items/${item.id}`"
    ...
    <NuxtLink :to="`/items/${item.id}`" class="block mt-2">
    ...
    <div v-if="showAdmin" class="mt-3 pt-3 border-t border-dashed border-stone-200 flex gap-2 justify-end">
...
const timeText = computed(() => {
  ...
  const today = shanghaiDateKey(iso) === shanghaiDateKey(new Date())
