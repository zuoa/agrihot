<template>
  <div class="bg-white rounded-2xl border border-leaf-100 p-6 sm:p-8 space-y-6">
    ...
    <p class="text-sm text-stone-600 leading-7">
      ...
      <NuxtLink to="/agent" class="text-leaf-700 hover:underline">开放推送接口</NuxtLink>
...
const api = useApi()
const { data: stats } = await useAsyncData('stats', () => api.stats().catch(() => null))
...
const sinceLabel = computed(() => {
  ...
  const key = shanghaiDateKey(stats.value.since)
  if (!key) return ''
  const [y, m] = key.split('-')
